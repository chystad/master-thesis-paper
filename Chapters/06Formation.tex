...
The first program block initializes the Monte Carlo simulation environment from the user-defined configuration file \texttt{monte\_carlo.yaml}. In this thesis, Monte Carlo execution is assumed to be enabled for all simulation cases, such that the simulator is structured around a queue of formation-flight simulation runs rather than a single isolated run. The Monte Carlo configuration defines the number of queued runs and the run-specific parameter variations used to generate override configuration files. If $n$ simulation runs are queued, $n-1$ override files are generated to allow variations in selected scenario parameters between otherwise identical simulations. Finally, the validated Monte Carlo settings are collected into a global Monte Carlo configuration instance, which is passed to the iterative part of the simulator and determines how the queued formation-flight simulations are executed.
